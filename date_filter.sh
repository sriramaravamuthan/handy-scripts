...
output_file=$4
suffix_start_time="T00:00:00"
suffix_end_time="T23:59:59"
rm -rf "$output_file"
while IFS=, read -r driverId startDateTime endDateTime
do
     ...
     then
        echo "$driverId","$input_start_date""$suffix_start_time","$input_end_date""$suffix_end_time" >> "$output_file"
     elif [[ "$rec_end_date" -le "$fmt_end_date" && "$rec_end_date" -ge "$fmt_start_date" ]];
     then
        echo "$driverId","$input_start_date""$suffix_start_time","$endDateTime" >> "$output_file"
     elif [[ "$rec_start_date" -ge "$fmt_start_date" && "$rec_start_date" -le "$fmt_end_date" ]];
     then
       echo "$driverId","$startDateTime","$input_end_date""$suffix_end_time" >> "$output_file"
     fi
done < "$input_file"
